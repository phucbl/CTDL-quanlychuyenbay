...
int NhapCB(nodeCB &ds, ChuyenBay &cb, listmb ds1){
	...
	while (i==1){
	 ...
	 //TangLuotBay(ds1,d);
	 strcpy(cb.trangthai,"1");cb.socot=ds1.nodes[d]->soday;cb.sodong=ds1.nodes[d]->sodong;
...
{	
	...
	cout << char(186);if (trth == 1) SetBGColor(1); cout << setw(3) << setfill('0') << text ; Normal(); cout<< char(186);Normal();
	//SetBGColor(Purple);
	gotoxy(x - 2, y + 1);
...
{
	int x = 20; int y = 8;
	int n = cb.socot*cb.sodong; int b=cb.socot;
	for (int i = 1; i <= n; i++)
	{	
		...
		BoxVe(x, y, text, cb.dsve[i - 1].trth); Normal();
		x += 5;
		if (i % b == 0)
		{
			y += 3;
...
{
	int current = index;
	int column = (current - 1) % cot;
	int row = (current - 1) / cot;
	char text[5]="";int a=current%cot;if (a==0) a=cot;int c=((current-1)/cot)+1;
		strcat(text,mangchucai[a]);strcat(text,mangchuso[c/10]);strcat(text,mangchuso[c%10]);
	SetBGColor(Red);
	gotoxy(13 + (column + 1) * 5 +1, (row + 1) * 3 + 5);
	cout << setw(3) << setfill('0') << text;
	Normal();strcpy(text,"");
	column = (currposVeT - 1) % cot;
	row = (currposVeT - 1) / cot;
	a=currposVeT%cot;if (a==0) a=cot; c=((currposVeT-1)/cot)+1;
		strcat(text,mangchucai[a]);strcat(text,mangchuso[c/10]);strcat(text,mangchuso[c%10]);
	if (preColor == Blue) SetBGColor(Blue);  else SetBGColor(Black);
	gotoxy(13 + (column + 1) * 5 +1, (row + 1) * 3 + 5);
	cout << setw(3) << setfill('0') << text;
	Normal();
	currposVeT = current;
 ...
 while(p != NULL)
   {
   	  ...
   	  int soVe = p->cb.socot*p->cb.sodong;
			for (int i = 0; i < soVe; i++)
				fwrite(&(p->cb.dsve[i]),sizeof(VE),1,f);
      p = p->next;
   }
